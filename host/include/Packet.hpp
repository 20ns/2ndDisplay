...
// Constants
constexpr size_t MAX_CHUNK_SIZE = 1350;
...
std::vector<std::vector<uint8_t>> createPackets(const std::vector<uint8_t>& frameData, 
    ...
    for (size_t i = 0; i < totalChunks; ++i) {
        ...
        const size_t remaining = frameData.size() - i * MAX_CHUNK_SIZE;
        const size_t chunkSize = (remaining < MAX_CHUNK_SIZE) ? remaining : MAX_CHUNK_SIZE;
        std::vector<uint8_t> packetData(HEADER_SIZE + chunkSize);
